...
#define TH_GENERIC_FILE "generic/SpatialConvolutionOneToOneMM.c"
#else

static void nn_(SpatialConvolutionOneToOneMM_updateOutput_frame)(THTensor *input, THTensor *output, THTensor *weight, THTensor *bias, THTensor *finput,
...
static int nn_(SpatialConvolutionOneToOneMM_updateOutput)(lua_State *L)
{
  ...
  if(input->nDimension == 3)
  {
    ...
    nn_(SpatialConvolutionOneToOneMM_updateOutput_frame)(input, output, weight, bias, finput,
                                                 kW, kH, dW, dH, padding,
...
#pragma omp parallel for private(t)
    for(t = 0; t < T; t++)
    {
      ...
      nn_(SpatialConvolutionOneToOneMM_updateOutput_frame)(input_t, output_t, weight, bias, finput_t,
                                                   kW, kH, dW, dH, padding,
                                                   nInputPlane, inputWidth, inputHeight,
                                                   nOutputPlane, outputWidth, outputHeight);
...
static int nn_(SpatialConvolutionOneToOneMM_updateGradInput)(lua_State *L)
{
  THTensor *input = luaT_checkudata(L, 2, torch_Tensor);
...
#pragma omp parallel for private(t)
    for(t = 0; t < T; t++)
    {
      ...
      nn_(SpatialConvolutionOneToOneMM_updateGradInput_frame)(gradInput_t, gradOutput_t, weight, fgradInput_t, kW, kH, dW, dH, padding);
...
static void nn_(SpatialConvolutionOneToOneMM_accGradParameters_frame)(THTensor *gradOutput, THTensor *gradWeight, THTensor *gradBias, THTensor *finput,
                                                              real scale)
{
  long i;
...
static int nn_(SpatialConvolutionOneToOneMM_accGradParameters)(lua_State *L)
{
  THTensor *input = luaT_checkudata(L, 2, torch_Tensor);
  THTensor *gradOutput = luaT_checkudata(L, 3, torch_Tensor);
  ...
  if(input->nDimension == 3)
  {
    nn_(SpatialConvolutionOneToOneMM_accGradParameters_frame)(gradOutput, gradWeight, gradBias, finput, scale);
  }
  else
  {
    long T = input->size[0];
    ...
    for(t = 0; t < T; t++)
    {
      ...
      nn_(SpatialConvolutionOneToOneMM_accGradParameters_frame)(gradOutput_t, gradWeight, gradBias, finput_t, scale);
...
static const struct luaL_Reg nn_(SpatialConvolutionOneToOneMM__) [] = {
  {"SpatialConvolutionOneToOneMM_updateOutput", nn_(SpatialConvolutionOneToOneMM_updateOutput)},
  {"SpatialConvolutionOneToOneMM_updateGradInput", nn_(SpatialConvolutionOneToOneMM_updateGradInput)},
  {"SpatialConvolutionOneToOneMM_accGradParameters", nn_(SpatialConvolutionOneToOneMM_accGradParameters)},
  {NULL, NULL}
};

static void nn_(SpatialConvolutionOneToOneMM_init)(lua_State *L)
{
  luaT_pushmetatable(L, torch_Tensor);
  luaT_registeratname(L, nn_(SpatialConvolutionOneToOneMM__), "nn");
  lua_pop(L,1);
}
